...
import org.example.data_objects.BaseDataObject;
...
public class DataObjectManager<T extends BaseDataObject> {
    ...
    public void addNewObject(T newObject){
        boolean idExists = objects.stream().anyMatch(o -> o.getId() == newObject.getId());

        if(idExists){
            throw new IllegalArgumentException("A object with that ID already exists" +
